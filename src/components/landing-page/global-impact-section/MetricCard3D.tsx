...
const AccentDecoration = ({ type }: { type: AccentType }) => {
  const decorations = {
    graph: (
      <svg
        className="w-full h-full absolute right-[-15%] top-[-5%] transform rotate-6"
        viewBox="0 0 200 200"
        aria-hidden="true"
      >
        <path
    ...
    globe: (
      <div className="absolute -top-12 -right-12 w-56 h-56 opacity-60 pointer-events-none">
        <svg
        ...
        >
          ...
          <ellipse
            ...
            rx="90"
            ry="30"
            className="animate-draw text-sky-400"
            strokeWidth="8"
            strokeDasharray="600"
          ...
          <ellipse
            ...
            rx="90"
            ry="30"
            className="animate-draw text-sky-300"
            strokeWidth="8"
            strokeDasharray="600"
            style={{ animationDelay: "2s", transform: "rotate(60deg)", transformOrigin: "center" }}
          />
          <ellipse
            cx="100"
            cy="100"
            ...
            strokeWidth="8"
            strokeDasharray="600"
            style={{ animationDelay: "3s", transform: "rotate(120deg)", transformOrigin: "center" }}
          />
        </svg>
      </div>
    ),
    bolt: (
      <div className="absolute top-0 right-0 p-8 w-48 h-48 opacity-60 pointer-events-none">
        <svg
  ...
  return (
    <div className="absolute top-0 right-0 w-full h-full opacity-[0.12] pointer-events-none">{decorations[type]}</div>
  );
};
...
export const MetricCard3D = ({ icon, accent, title, value, subtext, delay, children, className = "" }: MetricData) => {
  return (
    <div className={`relative group scale-65 ${className}`}>
      ...
      <div className="relative w-full aspect-3/2 transform-3d transform-[rotateX(35deg)_rotateZ(-5deg)_skewY(2deg)] transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:transform-[rotateX(30deg)_rotateZ(-3deg)_skewY(2deg)_translateY(-20px)]">
        {/* Glow Shadow */}
        ...
        {/* 3D Side */}
        <div className="absolute top-full left-0 w-full h-[40px] bg-linear-to-b from-blue-600 via-indigo-500 to-blue-400 rounded-b-[1.5rem] z-0 origin-top transform-[rotateX(-90deg)] overflow-hidden">
