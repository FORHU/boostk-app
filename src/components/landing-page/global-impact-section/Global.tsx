import { MetricCard3D } from "@/components/landing-page/global-impact-section/MetricCard3D";
import type { MetricData } from "@/enums/landing-page-enums";
import { ANIMATION_DELAYS, METRICS_DATA, STAGGER_CLASSES } from "@/enums/landing-page-enums";
import SectionHeader from "../SectionHeader";
import { ScrollReveal } from "./ScrollReveal";

const MetricCard = ({ metric, delay, stagger }: { metric: MetricData; delay: string; stagger: string }) => (
  <MetricCard3D
    icon={metric.icon}
      ...
      <div className="perspective-[2500px] relative w-full pt-2 -mt-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-0 gap-y-0 px-2 max-w-4xl mx-auto">
          {METRICS_DATA.map((metric, i) => (
            <MetricCard key={metric.title} metric={metric} delay={ANIMATION_DELAYS[i]} stagger={STAGGER_CLASSES[i]} />
          ))}
        </div>
      </div>
    </ScrollReveal>
