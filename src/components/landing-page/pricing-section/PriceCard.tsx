      ...
      >
        {badge && (
          <div className={`absolute ${highlight ? "-top-4 left-1/2 -translate-x-1/2" : "top-4 right-4"}`}>
            <span
              className={`${highlight ? "bg-primary text-white shadow-lg px-4 py-1.5 rounded-full" : "bg-blue-50 text-primary px-2 py-1 rounded border border-blue-100"} text-[10px] font-bold uppercase`}
            >
              {badge}
            </span>
          </div>
        ...
        <div
          className={`${highlight ? "bg-blue-50 border-blue-100" : "bg-slate-50 border-slate-100"} p-4 mb-6 border rounded-lg transition-colors duration-300`}
        >
          <p
            className={`text-[11px] font-bold uppercase tracking-widest mb-1 ${highlight ? "text-blue-600" : "text-slate-400"}`}
          >
            Best for:
          </p>
          ...
          <div className="grid grid-cols-2 gap-2">
            {icons.map((Icon) => (
              <div
                ...
                className="bg-slate-100 rounded h-12 flex items-center justify-center text-slate-400"
              >
                <Icon size={20} />
              </div>
            ))}
        ...
        <ul className="space-y-3 mb-10 flex-grow">
          {features.slice(0, 3).map((f) => (
            <li key={f} className="flex items-start gap-3 text-sm font-medium text-slate-600">
              <CheckCircle className="text-slate-400 mt-0.5 flex-shrink-0" size={18} />
              <span>{f}</span>
            </li>
          ))}
          <div className="pt-4 mt-2 border-t border-slate-100">
            ...
            <div className="space-y-2">
              {features.slice(3).map((f) => (
                <div key={f} className="flex items-start gap-3 text-xs font-medium text-slate-500">
