...
import { useSelectedTicket } from "@/components/chat-support/store";
import { TicketDetails } from "@/components/chat-support/ticket-details";
import { TicketList } from "@/components/chat-support/ticket-list";
    ...
    setTimeout(() => {
      ...
      if (status === "ARCHIVED") {
        if (selectedTicket?.id === ticketId) {
          // You might need an action in the store to clear it, but for now we won't mutate it here since we are keeping it simple
        }
  ...
  return (
    <div className="h-full flex flex-col">
      <Suspense fallback={<div className="p-4 h-14">Loading project tickets...</div>}>
        <TicketList projectId={project.id} />
      </Suspense>

      <div className="px-2 pb-2 flex-1 flex">
        {/* <div className="p-4">{selectedTicket ? selectedTicket.customer.name : "No ticket selected"}</div> */}
        <TicketWorkspace />
        {/* <TicketDetails
...
const TicketWorkspace = () => {
  ...
  if (!selectedTicket) {
    return <div className="border min-h-full flex-1 flex items-center justify-center">No ticket selected</div>;
  }

  return (
    <div className="grid grid-cols-4 minimal-scrollbar w-full h-[calc(100vh-9rem)] overflow-y-auto">
      <div className="border">
        <TicketDetails ticket={selectedTicket} />
      </div>
      <div className="border col-span-2">col 2 (2/4)</div>
      <div className="border">col 3 (1/4)</div>
    </div>
  );
};
