...
import { createMessageFn, getMessageHistoryFn } from "@/modules/message/message.serverFn";
...
export function ChatBox({ ticket }: { ticket: any }) {
  ...
  const currentUserId = useAppStore((state) => state.userAuth?.id) || context?.userAuth?.id || "agent_fallback";

  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
    queryKey: ["ticket-messages", ticket?.id],
    queryFn: async ({ pageParam }) => {
      try {
        const response = await getMessageHistoryFn({
          data: { ticketId: ticket.id, cursor: pageParam as string | undefined },
        });
        return response || { messages: [], nextCursor: undefined };
      } catch (error) {
        throw error;
      }
    },
