...
import { createOrgProjectFn, getOrgProjectsFn } from "@/modules/organization/organization.serverFn";

export const Route = createFileRoute("/(app)/org/$organizationId/")({
  component: RouteComponent,
  loader: async ({ params }) => {
    const projects = await getOrgProjectsFn({ data: { orgId: params.organizationId } });
    return { projects };
  },
});
...
function RouteComponent() {
  ...
  const filteredProjects = projects.filter((project: { id: string; name: string }) =>
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    await createOrgProjectFn({
      data: { orgId: organizationId, name: projectName },
    });
